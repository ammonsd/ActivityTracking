...
--------------------------------
-- User Dropdown Access Control
--
-- Modified by: Dean Ammons - February 2026
-- Change: Added all_users flag to dropdownvalues and new
--         user_dropdown_access table for client/project security
-- Reason: Restrict which clients and projects appear in dropdowns
--         based on the authenticated user. ADMIN role bypasses all
--         restrictions. Values with all_users = true are visible
--         to every user without needing a row in user_dropdown_access.
--------------------------------

-- Flag on dropdownvalues: when true, the value appears for ALL users
ALTER TABLE IF EXISTS public.dropdownvalues
    ADD COLUMN IF NOT EXISTS all_users BOOLEAN NOT NULL DEFAULT FALSE;

-- Maps a username to specific dropdown values they are permitted to see
CREATE TABLE IF NOT EXISTS public.user_dropdown_access (
    id                BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    username          VARCHAR(50) NOT NULL,
    dropdown_value_id BIGINT      NOT NULL,
    CONSTRAINT fk_uda_username
        FOREIGN KEY (username)
        REFERENCES public.users(username)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    CONSTRAINT fk_uda_dropdown
        FOREIGN KEY (dropdown_value_id)
        REFERENCES public.dropdownvalues(id)
        ON DELETE CASCADE,
    CONSTRAINT uk_uda
        UNIQUE (username, dropdown_value_id)
);

CREATE INDEX IF NOT EXISTS idx_uda_username
    ON public.user_dropdown_access (username);

CREATE INDEX IF NOT EXISTS idx_uda_dropdown_value_id
